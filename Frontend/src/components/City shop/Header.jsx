  ...
  return (
    <Sidebar>
      <SidebarContent className="bg-[var(--two2m)] ">
        <SidebarGroup>
          <SidebarGroupLabel className="font-bold text-2xl mb-3.5 text-[var(--two5m)]">
            CITY SHOP <Building2 className="ml-2 text-green-100" />
          </SidebarGroupLabel>
          <SidebarGroupContent>
            <SidebarMenu>
              {items.map((item) => (
                <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton
                    asChild
                    className="hover:bg-[var(--two5m)] flex  text-[var(--two5m)]"
                  >
                    <Link to={item.url}>
                      <item.icon />
                      <span className="flex ">
